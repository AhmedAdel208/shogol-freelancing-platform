...
import { createProjectSchema, type CreateProjectFormData } from "@/lib/validation/projectSchema";
...
export function useCreateProject() {
  ...
  const availableSkills = skillsData?.categories.flatMap(cat => cat.skills) || [];

  const form = useForm<CreateProjectFormData>({
    resolver: zodResolver<CreateProjectFormData, any, CreateProjectFormData>(createProjectSchema),
    defaultValues: {
      title: "",
      description: "",
      budget: 0,
  ...
  const { setValue, watch, handleSubmit, trigger, formState: { errors } } = form;
  ...
  const createMutation = useMutation({
    mutationFn: (formData: FormData) => jobRequestService.createJobRequest(formData),
    onSuccess: () => {
